...
public class Hechicero {
	...
	public String realizarPrediccionDeAventura() {
		...
		while (!puebloActual.equals(escenario.getDestino()) && avanzarAlSiguientePueblo(puebloActual)) {
			tiempoDeRecorrido += escenario.getMapaDePueblos().get(puebloActual).getCaminosAdyacentes()
					.get(recorrido.get(puebloActual)).getRecorridoEnDias();
	...
	private HashMap<String, String> dijkstra(String inicio) {
		...
		while (!cola.isEmpty()) {
			...
			if (!visitados.get(caminito.getPuebloDestino().getNombre())) {
				...
				for (Map.Entry<String, Camino> aristaAdyacente : caminito.getPuebloDestino().getCaminosAdyacentes()
						.entrySet()) {
					if (!visitados.get(aristaAdyacente.getKey())) {
						if (caminito.getRecorridoEnDias() + aristaAdyacente.getValue().getRecorridoEnDias() < distancias
								.get(aristaAdyacente.getKey())) {
							distancias.put(aristaAdyacente.getKey(),
									caminito.getRecorridoEnDias() + aristaAdyacente.getValue().getRecorridoEnDias());
							camino.put(aristaAdyacente.getKey(), caminito.getPuebloDestino().getNombre());
							cola.add(new Camino(aristaAdyacente.getValue().getPuebloDestino(),
									distancias.get(aristaAdyacente.getKey())));
						}
					}
				}
			}
